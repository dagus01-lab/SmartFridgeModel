...
from detector.utils import LOGGER, colorstr
from detector.utils.checks import check_version
from detector.utils.instance import Instances
from detector.utils.metrics import bbox_ioa
from detector.utils.ops import segment2box, xyxyxyxy2xywhr
from detector.utils.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
    ...
    def __call__(self, labels):
        ...
        if random.random() < self.probability:
            ...
            for _ in range(num_squares):
                square_size = random.randint(*self.square_size_range)
                x = random.random() * (width - square_size)
                y = random.random() * (height - square_size)

                draw.rectangle([x, y, x + square_size, y + square_size], fill=0)
            labels["img"] = np.asarray(img)
        ...
        if random.random() < self.probability:
            ...
            for y in range(height):
                for x in range(width):
                    if np.random.rand() < self.dot_probability:
